import "./Popup.css";
import ReinaDP from "@/assets/images/Hero-ligh/Hero-5.png";

export const RollerPopup = () => {
  return (
    <>
      <div className="popup-image">
        <img src={ReinaDP} alt="Placeholder Image" loading="lazy" />
      </div>
      <div className="popup-text__top">
        — Привет! Я — Рейна, и вы, наверное, уже знаете, что я не боюсь ни
        смерти, ни трудностей. Но даже мне иногда нужна поддержка. И знаете, что
        самое крутое? Мы справляемся со всем вместе. Сейчас я расскажу, как вы
        можете поддержать нас — команду «HeroSkAge» и компанию{" "}
        <span>ООО «Честный Эйб»</span>, которая создаёт нашу игру.
      </div>
      <hr className="popup-separator" />
      <h2 className="popup-title">Поддержка компании и разработчиков</h2>
      <div className="popup-text">
        Мы — не просто команда, мы — семья. И в этой семье каждый важен. Даже
        Эхо Стрит, моя подруга со школы, недавно вышла на связь, чтобы помочь
        нам в борьбе против «Eidovyrex». Это значит, что в игре вас ждёт ещё
        больше героев, историй и приключений! Но чтобы всё это стало
        реальностью, нам нужна ваша поддержка.
      </div>
      <p className="popup-subtitle">Вот как вы можете помочь:</p>
      <ol className="popup-list">
        <li>
          <strong>Расскажите о нас друзьям:</strong>
          <p>
            Поделитесь новостями о нашей игре в соцсетях, мессенджерах или
            просто в разговоре. Чем больше людей узнает о нас, тем сильнее мы
            станем.
          </p>
        </li>
        <li>
          <strong>Оставляйте отзывы и предложения:</strong>
          <p>
            Ваше мнение для нас бесценно. Напишите, что вам нравится, а что
            можно улучшить. Мы читаем каждое сообщение!
          </p>
        </li>
        <li>
          <strong>Поддержите финансово:</strong>
          <p>
            Если вы хотите помочь проекту материально, воспользуйтесь
            реквизитами в разделе Реквизиты. Каждый рубль пойдёт на развитие
            игры.
          </p>
        </li>
        <li>
          <strong>Следите за новостями:</strong>
          <p>
            Подпишитесь на наши обновления, чтобы первыми узнавать о выходе
            игры, новых героях и событиях.
          </p>
        </li>
      </ol>
      <hr className="popup-separator" />
      <p className="popup-title">Дополнительная информация от Рейны</p>
      <ul className="popup-list">
        <li>
          <strong>Мы справимся вместе:</strong>
          <p>
            В нашей команде каждый друг за друга. Мы все разные, но нас
            объединяет одна цель. И с вашей поддержкой мы станем ещё сильнее.
          </p>
        </li>
        <li>
          <strong>Эхо Стрит вернулась:</strong>
          <p>
            Да, моя подруга со школы, Эхо Стрит, снова с нами. Это значит, что в
            игре вас ждёт ещё больше крутых персонажей и неожиданных поворотов.
          </p>
        </li>
        <li>
          <strong>Спасибо за вашу веру в нас:</strong>
          <p>
            Мы знаем, что путь к успеху не бывает лёгким. Но с вашей поддержкой
            мы сможем всё. Ведь вместе мы — сила!
          </p>
        </li>
      </ul>
      <p className="popup-signature">
        С уважением,
        <br />
        Рейна «Адский роллер» Рэйдж 🎱
      </p>
      <div className="popup-text">
        P.S. Помните: даже в самом тёмном переулке можно найти свет, если идти
        вместе. А вы — наш свет. Спасибо, что вы с нами! 🌟
      </div>
    </>
  );
};
